...
class ComplianceEmailSystem:
    def __init__(self):
        ...
        print(f"Using SMTP server: {self.smtp_server}:{self.smtp_port} with user {self.smtp_username}")
    ...
    def load_database_data(self) -> bool:
        ...
        try:
            ...
            # Filter only pending tasks (open and in_progress)
    ...
    def load_completed_tasks_data(self) -> bool:
        """Load completed tasks data for reminder emails"""
        try:
            ...
            for _, task in completed_tasks.iterrows():
                ...
                enriched_tasks.append({
                    ...
                    'User Name': assigned_username,
                    'Attachment Link': task.get('attachment', ''),
                    'Status': task.get('status', 'closed'),
                    'Due Date': due_date,
                    'Frequency': task.get('frequency', 'One-time'),
                    'Completed Date': task.get('updated_at', datetime.now())
                })
            ...
            logger.info(f"Successfully loaded {len(self.data)} completed tasks across {self.data['Domain'].nunique()} domains")
            if len(self.data) > 0:
                logger.info(f"Users with completed tasks: {self.data['Email'].unique().tolist()}")
            return True
            
        except Exception as e:
            logger.error(f"Error loading completed tasks database: {e}")
            return False
    ...
    def get_overdue_tasks(self, days_overdue: int = 15) -> pd.DataFrame:
        ...
        try:
            ...
            if valid_dates_data.empty:
                return pd.DataFrame()
            
            # Use lowercase status comparison for safety
    ...
    def create_escalation_email_content(self, department_tasks: pd.DataFrame, department_head: str, domain: str) -> str:
        ...
        max_days_overdue = department_tasks['Days Overdue'].max() if not department_tasks.empty else 0
        ...
        for _, task in department_tasks.iterrows():
            ...
            html_content += f"""
                    <tr>
                        ...
                        <td class="{critical_class}">{task['Due Date'].strftime('%Y-%m-%d') if pd.notna(task['Due Date']) else 'N/A'}</td>
    ...
    def filter_tasks_by_schedule(self, schedule_type: str) -> pd.DataFrame:
        ...
        if schedule_type == "monthly":
            ...
            # Daily tasks - send every day for pending tasks
            ...
            # Weekly reminders - send every Monday for COMPLETED tasks only
            if today.weekday() != 0:
                logger.info("Not Monday - skipping weekly reminders")
                return pd.DataFrame()
            
            # For reminders, we want to send emails for completed tasks
            reminder_tasks = self.data[
                (self.data['Status'].str.lower() == 'closed')
            ]
            logger.info(f"Found {len(reminder_tasks)} completed tasks for reminders across {reminder_tasks['Domain'].nunique()} domains")
    ...
    def create_email_content(self, user_tasks: pd.DataFrame, schedule_type: str) -> str:
        ...
        if schedule_type == "reminder":
            email_type = "Weekly Completion Report"
        elif schedule_type == "daily":
            email_type = "Daily Task Reminder"
        elif schedule_type == "quarterly":
            email_type = "Quarterly Task Reminder"
        
        # Group tasks by domain for better organization
        tasks_by_domain = user_tasks.groupby('Domain')
        
        # Different content for reminders (completed tasks) vs other types (pending tasks)
        if schedule_type == "reminder":
            html_content = f"""
            <!DOCTYPE html>
            <html>
            <head>
                <style>
                    body {{ font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 20px; }}
                    .header {{ background-color: #d4edda; padding: 20px; text-align: center; border-radius: 5px; border-left: 6px solid #28a745; }}
                    ...
                    .task-table {{ width: 100%; border-collapse: collapse; margin: 10px 0; }}
                    .task-table th, .task-table td {{ border: 1px solid #ddd; padding: 12px; text-align: left; }}
                    .task-table th {{ background-color: #28a745; color: white; }}
                    .task-table tr:nth-child(even) {{ background-color: #f2f2f2; }}
                    .completed {{ color: #28a745; font-weight: bold; }}
                    .footer {{ margin-top: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 5px; }}
                    .summary {{ background-color: #d1ecf1; padding: 10px; border-radius: 5px; margin: 10px 0; }}
                </style>
            </head>
            <body>
                <div class="header">
                    <h2>🎉 Task Completion Report</h2>
                    <p>Hello {user_name}, you have successfully completed {task_count} task(s) across {len(tasks_by_domain)} domain(s)</p>
                </div>
            """
        else:
            html_content = f"""
        ...
        # Add quarterly notice if applicable (only for pending tasks)
        if schedule_type == "quarterly" and schedule_type != "reminder":
            html_content += """
                ...
                if schedule_type == "reminder":
                    ...
                    html_content += f"""
                            <tr>
                                ...
                                <td>{task['Due Date'].strftime('%Y-%m-%d') if pd.notna(task['Due Date']) else 'N/A'}</td>
                                <td>{task['Frequency']}</td>
                                <td><a href="{task['Attachment Link']}" target="_blank">View Files</a></td>
                            </tr>
                    """
                else:
                    # For pending tasks
                    days_remaining = (task['Due Date'] - datetime.now().date()).days if pd.notna(task['Due Date']) else 999
                    ...
                    html_content += f"""
                            <tr>
                                <td><strong>{task['Task']}</strong></td>
                                <td>{task['Task Description']}</td>
                                <td class="{urgent_class}">{task['Due Date'].strftime('%Y-%m-%d') if pd.notna(task['Due Date']) else 'Not set'} ({days_remaining} days remaining)</td>
                                <td>{task['Frequency']}</td>
        ...
        # Different footer for reminders vs other types
        if schedule_type == "reminder":
            html_content += f"""
                <div class="footer">
                    <p><strong>🎉 Great work!</strong> Thank you for completing these tasks.</p>
                    <p><strong>Note:</strong> This is an automated completion report. Please do not reply to this email.</p>
                    <p><strong>Domains Completed:</strong> {", ".join(tasks_by_domain.groups.keys())}</p>
                </div>
            </body>
            </html>
            """
    ...
    def process_tasks(self, schedule_type: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        # For reminders, load completed tasks; for others, load pending tasks
        if schedule_type == "reminder":
            if not self.load_completed_tasks_data():
                return {"success": False, "error": "Failed to load completed tasks data"}
        else:
            if not self.load_database_data():
                return {"success": False, "error": "Failed to load database data"}
        ...
        # Different email subjects based on schedule type
        if schedule_type == "reminder":
            email_subject = "Weekly Task Completion Report"
        elif schedule_type == "daily":
            email_subject = "Daily Compliance Task Reminder"
        elif schedule_type == "monthly":
            email_subject = "Monthly Compliance Task Report"
        elif schedule_type == "quarterly":
        ...
        if dry_run:
            ...
            for email, tasks in grouped_tasks:
                ...
                task_status = "completed" if schedule_type == "reminder" else "pending"
                print(f"   📨 To: {user_name} ({email}), {len(tasks)} {task_status} tasks")
        ...
        status_text = "completed" if schedule_type == "reminder" else "pending"
        logger.info(f"Processing complete: {emails_sent} emails sent, {emails_failed} failed, {result['domains_affected']} domains affected, {len(filtered_tasks)} {status_text} tasks")
        return result
...
def main():
    ...
    if len(args) != 1:
        ...
        print("Note: 'reminder' sends emails for COMPLETED tasks only")
        sys.exit(1)
    ...
    try:
        ...
        if result.get("success"):
            ...
            if schedule_type == "escalation":
                ...
                print(f"   Total {task_status} tasks: {result.get('total_tasks', 0)}")
                print(f"   Unique users: {result.get('unique_users', 0)}")
                print(f"   Domains affected: {result.get('domains_affected', 0)}")
        else:
