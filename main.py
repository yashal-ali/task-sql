...
#     # Check for OAuth callback - USING NEW API
...
#             if create_user(username, email, random_password, role):
...
#     # Page configuration
#     st.set_page_config(
#         page_title="TaskFlow Pro - Task Management",
#         page_icon="🚀",
#         layout="wide",
...
#     # Clear URL parameters after processing (for Microsoft auth) - USING NEW API
#     query_params = st.query_params
#     if query_params and st.session_state.logged_in:
#         st.query_params.clear()
...
from domain_operations import get_domains, add_domain, delete_domain, update_domain
from main_database import init_database
...
def generate_pdf_report(tasks_df, role):
    ...
    try:
        ...
        if 'domain' in tasks_df.columns and not tasks_df['domain'].isna().all():
            ...
            domain_counts = tasks_df['domain'].value_counts().head(6)
...
def show_analytics_dashboard(role):
    ...
    if role == 'admin':
        ...
        # Use email-based task fetching for Microsoft users
        if st.session_state.get('auth_method') == 'microsoft':
...
def set_page_styling():
    ...
    * {
    ...
    /* Add more styles as needed from your original styling */
    </style>
    """, unsafe_allow_html=True)
...
def show_admin_dashboard():
    ...
    with st.sidebar:
        ...
        auth_method = st.session_state.get('auth_method', 'traditional')
        auth_badge = "🏢 Microsoft" if auth_method == 'microsoft' else "🔐 Email"
        ...
        '>
            ...
            <h3 style='color: white; margin: 0 0 0.25rem 0; font-size: 1.125rem;'>{st.session_state.user_name}</h3>
            <p style='color: rgba(255, 255, 255, 0.8); margin: 0; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;'>Administrator • {auth_badge}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Import and call your existing functions
    if menu_action == "tasks":
        show_task_management()
    elif menu_action == "team":
        show_user_management()
    elif menu_action == "domains":
        show_domain_management()
    elif menu_action == "analytics":
        show_analytics_dashboard('admin')
    elif menu_action == "comments":
...
def show_user_dashboard():
    ...
    with st.sidebar:
        ...
        '>
            ...
            <h3 style='color: white; margin: 0 0 0.25rem 0; font-size: 1.125rem;'>{st.session_state.user_name}</h3>
            <p style='color: rgba(255, 255, 255, 0.8); margin: 0; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;'>Team Member • {auth_badge}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Quick user stats - use appropriate task fetching method
        if st.session_state.get('auth_method') == 'microsoft':
            tasks_df = get_tasks_for_microsoft_user(st.session_state.user_email)
        else:
            tasks_df = get_tasks(st.session_state.user_id, 'user')
...
def show_my_tasks():
    ...
    # Get tasks with appropriate method based on authentication
    if st.session_state.get('auth_method') == 'microsoft':
        tasks_df = get_tasks_for_microsoft_user(st.session_state.user_email, include_closed=True)
    else:
        tasks_df = get_tasks(st.session_state.user_id, 'user', include_closed=True)
    ...
    # Task filters and display logic (similar to your existing implementation)
    # ... [rest of your existing show_my_tasks implementation]

def show_my_performance():
    """Show personal performance metrics for the user"""
    st.title("📊 My Performance")
    
    # Get tasks with appropriate method
    if st.session_state.get('auth_method') == 'microsoft':
        tasks_df = get_tasks_for_microsoft_user(st.session_state.user_email)
    else:
        tasks_df = get_tasks(st.session_state.user_id, 'user')
    ...
    # Personal metrics and charts (your existing implementation)
    # ... [rest of your existing show_my_performance implementation]
...
def show_my_comments():
    ...
    # Display user's comments (your existing implementation)
    # ... [rest of your existing show_my_comments implementation]
...
def main():
    ...
    # Initialize session state with Microsoft auth support
    if 'logged_in' not in st.session_state:
        st.session_state.logged_in = False
    if 'user_id' not in st.session_state:
        st.session_state.user_id = None
